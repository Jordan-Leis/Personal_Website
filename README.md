...
Phones (viewport width 768 px or less) get a static gate page instead of the desktop: dark, green monospace text, a scrolling binary column (still under reduced motion), GitHub / Resume / LinkedIn links, and a muted “continue to the desktop anyway” link that sets a session flag and reloads. Nothing else initializes behind the gate (no boot, audio, or clock). Blog pages are unaffected. The browser suite therefore covers the desktop at 1440 px only, plus the gate at 390 px.

The dock opens or raises applications; clicking the focused application minimizes it. Orange dots remain for minimized applications. Alt+Tab cycles through open applications, including minimized ones. Escape dismisses the app grid or closes the focused window when keyboard focus is on the desktop. Window headers, desktop shortcuts, Files items, and cards share a Pointer Events lifecycle that ends on release, cancellation, lost capture, blur, or page hiding. Files keeps vertical touch scrolling on touch-screen desktops.
...
The system dropdown contains volume/mute controls and simulated connected `eduroam` and 87% battery indicators. Playback begins only through a user action. The subtle discovery tracker appears after the first discovery and turns gold at three, with space reserved for it below application windows.
