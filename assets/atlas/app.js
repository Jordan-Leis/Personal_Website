...
function renderSolution(){
  ...
  $('solution').innerHTML=`<div><h2 id="solution-title">${title}</h2>${verdict}<p>${instructions}</p><a class="open-game" href="https://linxicon.com/game/${data.game.id}?enterGame=">Open game #${data.game.id}</a><div class="solution-links" aria-label="Inspect each link">${links}</div></div><div><h3>${data.candidates.length===1?'The only shortest chain':'Shortest chains found'}</h3><ol class="alternates">${alternates}</ol><p>Same number of links, ordered by total local score. Each chain shows its own result from the game.</p></div>`;
}
